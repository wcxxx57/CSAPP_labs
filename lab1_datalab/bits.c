...
int isTmax(int x) {
  ...
  int neg_reverse_x=(~reverse_x)+1;
  return !((neg_reverse_x^reverse_x)|(!reverse_x));//check取反是否是tmin
}
/* 
...
int isLessOrEqual(int x, int y) {
  ...
  int neg_x=(~x)+1;
  int dif=y+neg_x;//y-x
  int sign_dif=(dif>>31)&1;
  //考虑y-x会有溢出 要先检查符号位进行大小比较
...
int logicalNeg(int x) {
  int neg_x=(~x)+1;//利用0是唯一和其相反数符号相同的数 
  return ((x|neg_x)>>31)+1;
}
...
int howManyBits(int x) {
  /*需要用到的mask*/
  int m1=~(((~1)+1)<<16);//0000111
  int m2=(m1<<8)^m1;
  int m3=(m2<<4)^m2;
  int m4=(m3<<2)^m3;
  int m5=(m4<<1)^m4;//010101 
  /*处理符号*/
  int sign=x>>31;
  x=x^((x^(~x))&sign);//no-branch minimum的方法处理负数
  /*求最高位1的位数*/
  //拓展最高位1
  x|=x>>1;
  x|=x>>2;
